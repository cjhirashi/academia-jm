...
export function ContactoSection({ contacto }: { contacto?: Contacto | null }) {
  ...
  return (
    <section id="contacto" className="py-20 px-4 bg-[#0F0D13]">
      <div className="mx-auto max-w-6xl">
        <div className="mb-12">
          <p className="text-[12px] font-medium tracking-[0.5px] uppercase text-[var(--gold)] mb-3">Contáctanos</p>
          <h2 className="text-[32px] font-normal leading-[40px] text-[#E6E0E9]">¿Listo para empezar?</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          <motion.div
            ...
            className="bg-[#211F26] rounded-[28px] p-8"
          >
            {!c && (
              <p className="text-[#938F99] text-[14px] py-4">Información de contacto próximamente...</p>
            )}
            ...
            {c && items.map(({ icon: Icon, label, key }, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-start gap-4 py-5 border-b border-[#49454F] last:border-0"
                >
                  <div className="w-10 h-10 rounded-full bg-[var(--gold)]/12 flex items-center justify-center shrink-0">
                    <Icon className="h-4 w-4 text-[var(--gold)]" />
                  </div>
                  <div>
                    <p className="text-[11px] font-medium tracking-[0.5px] uppercase text-[#938F99] mb-1">{label}</p>
                    <p className="text-[14px] leading-[20px] tracking-[0.25px] text-[#E6E0E9]">{value}</p>
                  </div>
                </motion.div>
              )
            })}
            ...
            {c && c.facebook_url && (
              <div className="pt-6">
                <a href={c.facebook_url} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-[14px] font-medium text-[#CAC4D0] hover:text-[var(--gold)] transition-colors">
